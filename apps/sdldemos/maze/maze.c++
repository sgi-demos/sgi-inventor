...
/* SDL port: overlay-plane Inventor logo removed (no overlay planes). */

int
#ifdef __EMSCRIPTEN__
// Ubuntu's emscripten 3.1.6 + clang 15 don't agree on the symbol name
// for two-arg main (__main_argc_argv), which silently strips the whole
// program. The browser has no argv, so use the zero-arg form there.
main()
{
...
main(int argc, char *argv[])
{
    ...
    SoSDLExaminerViewer *vwr = new SoSDLExaminerViewer("Inventor Maze", 900, 700);
    ...
    if (getenv("MAZE_DUMP_SCENE")) {     // debug: write assembled scene
        SoOutput out;
        out.openFile(getenv("MAZE_DUMP_SCENE"));
        SoWriteAction wa(&out);
        wa.apply(mainRoot);
        out.closeFile();
    }
    vwr->setBackgroundColor(SbColor(0.25f, 0.25f, 0.35f));
    vwr->setSceneGraph(mainRoot);
    vwr->setViewing(false);    // picking mode: game handles all input

    // Set the overlay graph - Inventor logo
    // setOverlayLogo(vwr);  // SDL port: no overlay planes
