...
public:
  ...
  uint32_t getDataId() const { return _nodeId; }
...
private:
  ...
  void allocateData(int numBytes, int nodeId, SoState* state);

  //! allocates new data buffer, copies the given values and swaps them bytewise, needed for little endian machines
  //! sets hasSwappedRGBAData() flag
  void copyAndSwapPackedRGBA(int numValues, const uint32_t* values, int nodeId, SoState* state);
