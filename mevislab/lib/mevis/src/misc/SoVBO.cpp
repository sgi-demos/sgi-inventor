...
bool SoVBO::shouldUseVBO( SoState* state , int numData )
{
  ...
  bool result = isVBORenderingAllowed && GLEW_ARB_vertex_buffer_object && (numData > _vboMinimumSizeLimit) && (numData < _vboMaximumSizeLimit);
  if (state) {
...
void SoVBO::setData(int numBytes, const void* data, int nodeId, SoState* state)
{
...
void SoVBO::allocateData( int numBytes, int nodeId , SoState* state)
{
  if (_ownsData && _data) {
    free((void*)_data);
  }
...
void SoVBO::copyAndSwapPackedRGBA( int numValues, const uint32_t* values, int nodeId, SoState* state )
{
...
{
  if (!_glBuffer || SoGLCacheContextElement::get(state)!=_glBuffer->getContext()) {
    if (_glBuffer) {
      _glBuffer->unref(state);
    }
