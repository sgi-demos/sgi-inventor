...
class INVENTOR_API SoKeyboardEvent : public SoButtonEvent {
  ...
  public:
    ...
    enum Key {
        ...
        ESCAPE      = 0xff1b, 
        DELETE      = 0xffff, 
        //! Several Windows headers define DELETE, better to use a different name
        //! Also added for TGS Open Inventor compatibility
        KEY_DELETE  = DELETE,
        PRINT       = 0xff61, 
        INSERT      = 0xff63, 
        NUM_LOCK    = 0xff7f, 
    ...
    void                setIsAutoRepeat(SbBool b) { autoRepeat = b; }

    //! get auto-repeat flag (see notes for setIsAutoRepeat); MEVIS Inventor only
    SbBool              isAutoRepeat() const      { return autoRepeat; }
