...
static FcBool fontconfigInitialized = FcFalse;
#else
...
# if defined(MAC_OS_X_VERSION_10_7) && (MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_7)
  if (!path) {
    ...
    /* Try to find font by using its postscript name (other font name types are matched in a fallback manner) */
    CTFontRef font = CTFontCreateWithName(name, 0.0, NULL);
    CFRelease(name);
    if (!font) {
      /* Use fallback font */
      font = CTFontCreateWithName(CFSTR("Times-Roman"), 0.0, NULL);
    }
    if (font) {
