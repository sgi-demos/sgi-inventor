...
{
#ifdef SDL_PORT
    printf("\nPaint By Numbers - fill squares so each row/column matches\n"
	   "its clue numbers (runs of filled squares, in order).\n"
	   "  left click/drag   fill squares\n"
	   "  middle click/drag mark squares empty\n"
	   "  right click/drag  clear squares back to unknown\n"
	   "  u undo   r redo   c clear   t test solvability\n"
	   "  1 solve one line  a solve all   h this help   q quit\n\n");
#else
...
keyboardCB(void *, SoEventCallback *ecb)
{
    ...
    else if (SO_KEY_PRESS_EVENT(ev, NUMBER_1)) { solveOneCB(NULL, NULL, NULL); }
    else if (SO_KEY_PRESS_EVENT(ev, A))	 { solveAllCB(NULL, NULL, NULL); }
...
// Zero-arg main (as in the other games); default puzzle from the
// preloaded data.
int
main()
{
    int argc = 2;
    static char *argv[] = { (char *)"pbnsolve",
			    (char *)"puzzles/0.pbn", NULL };
#else
int
main(int argc, char **argv)
{
...
#ifdef SDL_PORT
    // Bundled substitute font for the spec numbers (data/fonts/).
    ...
    printf("Paint By Numbers - press 'h' in the window for help.\n");
#else
    buildWidget();
