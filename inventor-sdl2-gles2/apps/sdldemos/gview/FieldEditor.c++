...
//
// Console/text-prompt port of the Motif field editor. See FieldEditor.h.
//
...
FieldEditor::FieldEditor(SoNode *node, Widget, const char *)
//
////////////////////////////////////////////////////////////////////////
{
    nodeToEdit = node;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    printf("Edit fields by typing '<name> <value>' (e.g. 'width 3').\n"
	   "  !default <name>   set field to its default value\n"
	   "  !ignore <name>    toggle the field's Ignore flag\n"
	   "  !override         toggle the node's Override flag\n"
	   "  !list             print the fields again\n"
	   "  empty line or Esc finishes editing\n");

    TextPrompt::activate("field>", &FieldEditor::lineCB,
			 &FieldEditor::cancelCB, this);
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    printf("=== %s fields ===%s\n",
	   nodeToEdit->getTypeId().getName().getString(),
	   nodeToEdit->isOverride() ? "  [override]" : "");

    for (i = 0; i < numFields; i++) {
	SoField		*field = fdata->getField(nodeToEdit, i);
	SbString	value;

	field->get(value);

	printf("  %-18s = %s%s%s\n",
	       fdata->getFieldName(i).getString(),
	       value.getString(),
	       field->isIgnored() ? "  [ignored]" : "",
...
SoField *
FieldEditor::findField(const char *name, int *indexOut)
//
////////////////////////////////////////////////////////////////////////
{
    const SoFieldData	*fdata = nodeToEdit->getFieldData();
    ...
    for (i = 0; i < numFields; i++) {
	if (strcmp(fdata->getFieldName(i).getString(), name) == 0) {
	    if (indexOut != NULL)
		*indexOut = i;
	    return fdata->getField(nodeToEdit, i);
	}
    }
...
SbBool
FieldEditor::applyLine(const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    // Skip leading blanks
    ...
    // An empty line finishes editing
    if (*line == '\0')
	return TRUE;

    // !-commands cover the dialog's buttons
    if (line[0] == '!') {
	char	cmd[64], name[128];
	int	n = sscanf(line + 1, "%63s %127s", cmd, name);

	if (n >= 1 && strcmp(cmd, "list") == 0)
	    printFields();

	else if (n >= 1 && strcmp(cmd, "override") == 0) {
	    nodeToEdit->setOverride(! nodeToEdit->isOverride());
	    printf("override is now %s\n",
		   nodeToEdit->isOverride() ? "on" : "off");
	}

	else if (n == 2 && strcmp(cmd, "ignore") == 0) {
	    SoField	*field = findField(name);
	    if (field == NULL)
		fprintf(stderr, "No field named \"%s\"\n", name);
	    else {
		field->setIgnored(! field->isIgnored());
		printf("%s ignore flag is now %s\n", name,
		       field->isIgnored() ? "on" : "off");
	    }
	}

	else if (n == 2 && strcmp(cmd, "default") == 0) {
	    int		index;
	    SoField	*field = findField(name, &index);
	    if (field == NULL)
		fprintf(stderr, "No field named \"%s\"\n", name);
	    else {
		// Get the value from a node instance with all default
		// values and store it into the edited node
		SoField	 *defField =
		    defNode->getFieldData()->getField(defNode, index);
		SbString value, setStr;
		defField->get(value);
		setStr  = name;
		setStr += " ";
		setStr += value;
		nodeToEdit->set(setStr.getString());
	    ...
	    fprintf(stderr,
		    "Unknown command; !list !default !ignore !override\n");

	return FALSE;
    }

    // "<fieldName> <value...>" - the same format the Motif editor's
    // Apply button built for SoNode::set().

    // Set the read-error callback so that read errors caused by the
    // following call to SoNode::set() can be trapped and displayed
    SoReadError::setHandlerCallback(&FieldEditor::readErrorCB, this);

    if (! nodeToEdit->set(line))
	displayReadError();

    else {
	// Echo the resulting field value back
	char	name[128];
    ...
    // Restore the default read-error handler
...
//
// Description:
//    TextPrompt callback for a completed line.
//

void
FieldEditor::lineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    FieldEditor	*editor = (FieldEditor *) userData;

    if (editor->applyLine(line)) {
...
//
// Description:
//    TextPrompt callback for Escape.
//

void
FieldEditor::cancelCB(void *userData)
//
////////////////////////////////////////////////////////////////////////
{
    FieldEditor	*editor = (FieldEditor *) userData;
...
//
// Description:
//    Invokes the finish callback and deletes the editor (the Motif
//    editor's windowCloseAction).
//
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (editor->errorString == NULL) {

	SbString *err = new SbString;

	(*err) = "There was an error reading the field value:\n";
	(*err) += error->getDebugString();

	editor->errorString = err;
    }
}
...
{
    if (errorString != NULL) {
	new Error(NULL, errorString->getString());
	delete errorString;
