...
// Text-prompt port of the Motif selection dialog: the user types the
// node class name in the graph window ('?' lists all classes).
class NodeCreator {
  ...
  private:
    ...
    // Prints the names of all known node classes
    void		printNodeNames();
    ...
    // name was invalid (the prompt stays up).
    SbBool		invokeCallback(const char *nodeName);
};
