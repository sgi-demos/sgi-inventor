...
//
// Text-prompt port: the Motif selection dialog becomes a typed-in node
// class name (see NodeCreator.h).
//
...
//
// Description:
//    Constructor - takes parent widget (ignored) and callback to call
//    when a node name is entered.
//

NodeCreator::NodeCreator(Widget,
...
////////////////////////////////////////////////////////////////////////
{
    ...
    printf("Create node: type the class name (e.g. Cube)."
	   " '?' lists all classes, Esc cancels.\n");

    TextPrompt::activate("create>", &NodeCreator::lineCB,
			 &NodeCreator::cancelCB, this);
}
...
void
NodeCreator::lineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    NodeCreator	*creator = (NodeCreator *) userData;

    if (strcmp(line, "?") == 0) {
	creator->printNodeNames();
	return;
    }

    // An empty line cancels, like the dialog's Cancel button. An
    // invalid name leaves the prompt up, like the original dialog.
    if (creator->invokeCallback(line[0] == '\0' ? NULL : line)) {
...
void
NodeCreator::cancelCB(void *userData)
//
////////////////////////////////////////////////////////////////////////
{
    NodeCreator	*creator = (NodeCreator *) userData;
...
//
// Description:
//    Prints the names of all known node classes (the dialog's list).
//

void
NodeCreator::printNodeNames()
//
////////////////////////////////////////////////////////////////////////
{
    SoTypeList	allNodes;
    const char	**typeNames;
    int		numTypes, numStrings, curType, i;
    SoType	type;
    ...
    printf("Available node types:\n");
    for (i = 0; i < numStrings; i++)
	printf("  %-24s%s", typeNames[i],
	       (i % 3 == 2 || i == numStrings - 1) ? "\n" : "");
...
//
// Description:
//    Used to sort the node names for printNodeNames().
//
...
//    function. The name may be a NULL pointer. Returns FALSE if the
//    name did not name a creatable node class.
//
