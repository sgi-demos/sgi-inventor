...
//
// SDL port: the graph viewer is its own window (an SoSDLRenderArea
// subclass) instead of an SoXtPlaneViewer half of a form. The Motif
// pulldown menus become single-key shortcuts handled in processEvent
// (see printHelp); the plane viewer's pan/zoom become Esc-toggled
// left-drag panning, middle-drag panning and mouse-wheel zooming on
// the display graph's orthographic camera.
//

class GraphViewer : public SoSDLRenderArea {
 public:
    GraphViewer(const char *title = "gview graph",
		int width = 640, int height = 520);
    ~GraphViewer();
    ...
    // Keyboard shortcuts, panning and zooming, and routing to an
    // active text prompt (field editor / node creator)
  ...
  private:
    ...
    SbBool		panMode;	// TRUE if left-drag pans (Esc toggles)
    SbBool		panning;	// TRUE during a pan drag
    SbVec2s		panLast;	// Last pan drag position
    ...
    // Zooms the orthographic camera by the given factor
    void		zoomBy(float factor);

    // Makes the whole display graph visible (plane viewer's viewAll)
    void		viewAllGraph();
