...
#ifdef SDL_PORT
    // The compiled-in icon header is generated by the ivToIncludeFile
    // host tool, which cannot run during a cross (wasm) build. The SDL
    // port ships gviewIcons.iv beside the binary instead and reads it
    // here, as the original code (below) once did.
