...
#ifdef SDL_PORT
GraphViewer::GraphViewer(const char *title, int width, int height)
		: SoSDLRenderArea(title, width, height)
//
////////////////////////////////////////////////////////////////////////
{
    userScene	 = NULL;
...
#ifdef SDL_PORT
    // The Xt plane viewer supplied a headlight above the scene; the
    // plain render area does not, so light the icons here (the
    // camera never rotates, so a fixed light is equivalent)
    SoDirectionalLight *headlight = new SoDirectionalLight;
    headlight->direction.setValue(.2, -.2, -.98);
    displayRoot->addChild(headlight);
#endif
...
#ifdef SDL_PORT
    SoSDLRenderArea::setSceneGraph(displayRoot);
#else
...
////////////////////////////////////////////////////////////////////////
{
	...
	break;
      ...
      case NODE_CREATE:
#ifdef SDL_PORT
	nc = new NodeCreator(NULL, &GraphViewer::nodeCreationCB, this);
	(void) nc;
#else
...
#ifdef SDL_PORT
    // The overlay is drawn as an RGBA second pass here, so overlay
    // color-index 2 (bright yellow in the color map below) becomes a
    // base color
    ci = new SoBaseColor;
    ci->rgb.setValue(1.0, 1.0, 0.0);
#else
    ci = new SoColorIndex;
    ci->index = 2;
#endif
...
#ifdef SDL_PORT
    SoCamera		*cam = camera;
    const SbViewportRegion	vpReg = getViewportRegion();
#else
...
//
// Description:
//    Handles SDL events. An active text prompt (field editor / node
//    creator) owns the keyboard; single-key shortcuts replace the
//    Motif menus; Esc-toggled left-drag panning, middle-drag panning
//    and wheel zooming replace the plane viewer's camera controls.
//    Everything else goes to the scene graph (selection, pasting).
//

void
GraphViewer::processEvent(const SDL_Event *event)
//
////////////////////////////////////////////////////////////////////////
{
    if (TextPrompt::handleEvent(event))
	return;
	...
	break;
      ...
      case SDL_MOUSEWHEEL:
	zoomBy(event->wheel.y > 0 ? 1.0 / 1.1 : 1.1);
	return;

      case SDL_MOUSEBUTTONDOWN:
	if (event->button.button == SDL_BUTTON_MIDDLE ||
	    (panMode && event->button.button == SDL_BUTTON_LEFT)) {
	    panning = TRUE;
	    panLast.setValue(event->button.x, event->button.y);
	    return;
	}
	break;
	...
	if (panning) {
	    panBy(event->motion.x - panLast[0],
		  event->motion.y - panLast[1]);
	    panLast.setValue(event->motion.x, event->motion.y);
	    return;
	}
	break;
    ...
    SoSDLRenderArea::processEvent(event);
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//    Handles a key press; returns TRUE if it mapped to an action.
//    The keys mirror the accelerators of the Motif menus (Menu.h),
//    without the Alt modifier.
//

SbBool
GraphViewer::handleKey(const SDL_Event *event)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    switch (sym) {
      ...
      case SDLK_ESCAPE:
	panMode = ! panMode;
	printf("graph window: %s mode\n",
	       panMode ? "pan (left-drag pans)" : "select");
	return TRUE;

      case SDLK_u:	action = GRAPH_UPDATE;				break;

      case SDLK_x:	action = EDIT_CUT;				break;
      case SDLK_c:	action = EDIT_COPY;				break;
      case SDLK_v:	action = shift ? EDIT_PASTE_REF : EDIT_PASTE;	break;
      case SDLK_d:	action = shift ? EDIT_DUP_REF	: EDIT_DUP;	break;
      case SDLK_BACKSPACE:
      case SDLK_DELETE:	action = EDIT_DELETE;				break;

      case SDLK_LEFT:	action = SELECT_LEFT_SIB;			break;
      case SDLK_RIGHT:	action = SELECT_RIGHT_SIB;			break;
      case SDLK_UP:	action = SELECT_PARENT;				break;
      case SDLK_DOWN:	action = SELECT_1ST_CHILD;			break;

      case SDLK_o:	action = shift ? GROUP_OPEN_ALL : GROUP_OPEN;	break;
      case SDLK_k:	action = GROUP_CLOSE;				break;
      case SDLK_t:	action = GROUP_TOGGLE;				break;

      case SDLK_i:	action = INST_TOGGLE;				break;
      case SDLK_w:	action = INST_SWAP;				break;

      case SDLK_n:	action = NODE_CREATE;				break;
      case SDLK_f:	action = NODE_EDIT;				break;

      default:
	return FALSE;
    }

    processAction(action, ecb);
...
//
// Description:
//    Pans the orthographic camera by a pixel delta (dragging the
//    graph along with the cursor).
//

void
GraphViewer::panBy(int dx, int dy)
//
////////////////////////////////////////////////////////////////////////
{
    if (camera == NULL)
    ...
    // World units per pixel at the current zoom
    float	wpp = camera->height.getValue() / getSize()[1];
    SbVec3f	p   = camera->position.getValue();

    camera->position.setValue(p[0] - dx * wpp, p[1] + dy * wpp, p[2]);
...
//
// Description:
//    Zooms the orthographic camera by the given factor (the plane
//    viewer's zoom slider changed the view volume size the same way).
//

void
GraphViewer::zoomBy(float factor)
//
////////////////////////////////////////////////////////////////////////
{
    if (camera == NULL)
    ...
    camera->height = camera->height.getValue() * factor;
...
//
// Description:
//    Makes the whole display graph visible (the viewer's viewAll).
//

void
GraphViewer::viewAllGraph()
//
////////////////////////////////////////////////////////////////////////
{
    if (camera == NULL || displayRoot == NULL)
	return;

    camera->viewAll(displayRoot, getViewportRegion());
...
////////////////////////////////////////////////////////////////////////
{
    printf("Graph window keys (the Motif menus):\n"
	   "  u          update graph from the scene      (Graph menu)\n"
	   "  x / c      cut / copy selected node         (Edit menu)\n"
	   "  v / V      paste / paste reference - then click where to attach\n"
	   "  d / D      duplicate / duplicate by reference\n"
	   "  Backspace  delete selected node\n"
	   "  arrows     select left/right sibling, parent (Up),\n"
	   "             first child (Down)                (Select menu)\n"
	   "  o / O / k  open group / open all / close     (Group menu)\n"
	   "  t          toggle group open-closed (also double-click)\n"
	   "  i / w      instance: show connection / swap  (Inst menu)\n"
	   "  n          create a node: type its class name, then click\n"
	   "  f          edit fields of selected node (type in this window;\n"
	   "             double-click on a non-group icon does this too)\n"
	   "  Esc        toggle pan mode (left-drag pans); middle-drag\n"
	   "             always pans; mouse wheel zooms\n");
}
