...
SoSDLRenderArea	*XStuff::component = NULL;
Display		*XStuff::display;      // unused under SDL
Cursor		*XStuff::cursors = NULL;

// Build an SDL cursor from the original X11 bitmap (LSB-first rows;
// SDL_CreateCursor wants MSB-first). The bitmap doubles as its own
// mask: set bits draw black, clear bits are transparent.
static SDL_Cursor *
makeSDLCursor(const unsigned char *bits, int w, int h, int xhot, int yhot)
{
    int rowBytes = (w + 7) / 8;
    static unsigned char rev[16] = { 0x0,0x8,0x4,0xC,0x2,0xA,0x6,0xE,
				     0x1,0x9,0x5,0xD,0x3,0xB,0x7,0xF };
    unsigned char data[3*24], mask[3*24];
    for (int i = 0; i < rowBytes*h; i++) {
	unsigned char b = bits[i];
	unsigned char m = (unsigned char)((rev[b & 0xF] << 4) | rev[b >> 4]);
	data[i] = m;
	mask[i] = m;
    }
    return SDL_CreateCursor(data, mask, rowBytes*8, h, xhot, yhot);
}
#else
...
{
#ifdef SDL_PORT
    if (component && component->getWindow())
	SDL_WarpMouseInWindow(component->getWindow(), loc[0], loc[1]);
#else
