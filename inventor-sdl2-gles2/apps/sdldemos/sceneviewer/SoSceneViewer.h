...
class SvViewer;		// the examiner-viewer window (defined in the .c++)
class SvClipboard;	// in-process copy/paste buffer (defined in the .c++)

//
// SDL port: SoSceneViewer is a plain class managing one examiner-viewer
// window. The Motif menu bar becomes single-key shortcuts dispatched
// from the viewer window through handleKey()/processMenuAction() using
// the same SV_* menu ids; the Motif editor components (material/color
// editors, transform sliders, light editors) become the typed
// field-editor prompt shared with the gview port; the file selection
// dialog becomes a typed file-name prompt. The manipulators - the
// heart of SceneViewer - are pure Inventor and unchanged.
//
...
class SoSceneViewer {
  ...
  public:
    ...
    // Copies the given camera's values into the viewer camera
    void    	setCamera(SoCamera *cam)	{ setCameraFields(cam); }
    SoCamera	*getCamera();

    void	        newData();

    // The window exists as soon as it is created
    void		show()			{}
    ...
    // Keyboard dispatch from the viewer window; returns TRUE if the
    // key mapped to a menu action
    SbBool		handleKey(const SDL_Event *event);

    // Performs the menu action with the given SV_* id
    void		processMenuAction(int id);
 ...
 private:
    ...
    int		fileMode;
    char	*fileName;
    void	getFileName();		// typed file-name prompt
    ...
    // TextPrompt callbacks for the file-name prompt
    static void	fileLineCB(void *userData, const char *line);
    static void	fileCancelCB(void *userData);
    ...
    // printing is not available in the SDL port
    void    	    	print();
    ...
    // for copy and paste (in-process byte-stream buffer)
    SvClipboard		*clipboard;
    ...
    // Viewing
    //
    SvViewer		*currentViewer;	// the one viewer (Examiner)
    ...
    SbBool		antialiasingFlag;	// unavailable; kept for state
    void		setAntialiasing(SbBool onOrOff);

    // current transparency type
    SoGLRenderAction::TransparencyType transpType;

    // Background color (typed prompt)
    void		editBackgroundColor();
    static void	    	backgroundColorLineCB(void *userData, const char *line);

    //
    // Editors (typed field-editor prompts)
    //
    ...
    static void		diffuseColorLineCB(void *userData, const char *line);
    static void		promptCancelCB(void *userData);
    ...
    static void adjustScaleTabSizeCB( void *, SoSDLExaminerViewer *);
    ...
    // Ambient lighting color (typed prompt)
    void		editAmbientColor();
    static void	    	ambientColorLineCB(void *userData, const char *line);
