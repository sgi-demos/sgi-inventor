...
#define SoXtViewer SoSDLExaminerViewer
#define SoXtFullViewer SoSDLExaminerViewer
...
class SvLightData {
  public:
    ...
    char	    *name;
#ifdef SDL_PORT
    void	    *colorEditor;	// no Motif color editor here
#else
...
//
// The one viewer window: an examiner viewer that first offers events
// to an active text prompt, then to the SceneViewer's menu shortcuts.
//
class SvViewer : public SoSDLExaminerViewer {
    SoSceneViewer *sv;
  public:
    SvViewer(const char *t, int w, int h, SoSceneViewer *s)
	: SoSDLExaminerViewer(t, w, h), sv(s) {}
    virtual void processEvent(const SDL_Event *e) {
	if (TextPrompt::handleEvent(e))
	    return;
	if (e->type == SDL_KEYDOWN && sv->handleKey(e))
	    return;
...
//
// In-process replacement for SoXtClipboard: copy runs the selected
// paths through SoByteStream (a deep copy, exactly what the X
// clipboard transferred); paste unconverts them back into fresh paths.
//
class SvClipboard {
    SoByteStream stream;
    SbBool	 full;
  public:
    SvClipboard()			{ full = FALSE; }
    void copy(SoPathList *paths) {
	stream.convert(paths);
	full = TRUE;
    }
    void paste(void (*doneCB)(void *, SoPathList *), void *userData) {
	if (! full)
	    return;
	SoPathList *pathList =
	    SoByteStream::unconvert(stream.getData(), stream.getNumBytes());
	if (pathList != NULL && pathList->getLength() > 0)
	    doneCB(userData, pathList);	// doneCB deletes pathList
	else
	    delete pathList;
    }
};
...
#ifdef SDL_PORT
static void
setOverlayLogo(SoSDLRenderArea *ra)
#else
static void
...
////////////////////////////////////////////////////////////////////////
//
// Public constructor - build the viewer window right now
//
...
////////////////////////////////////////////////////////////////////////
//
// Called by the constructor (SDL port: no X resources, no Motif menu
// data - the tables in SoSceneMenu.h already carry their labels)
//
// private
//
void
...
////////////////////////////////////////////////////////////////////////
{
    ...
    transpType = SoGLRenderAction::SCREEN_DOOR;
    ...
    // do this after everything else has been set up (viewer needs to
    // exist because the camera is gotten from the viewer)
    if (envFile != NULL)
    	readEnvFile(envFile);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // detach and delete the viewer
    if (currentViewer != NULL)
	currentViewer->setSceneGraph(NULL);
    delete (SoSDLExaminerViewer *) currentViewer;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // Only the examiner viewer exists in the SDL port.
    switch (newViewer) {
	case SV_VWR_FLY:
	    printf("The Fly viewer is not available in the SDL port.\n");
	    break;
	case SV_VWR_WALK:
	    printf("The Walk viewer is not available in the SDL port.\n");
	    break;
	case SV_VWR_PLANE:
	    printf("The Plane viewer is not available in the SDL port.\n");
	    break;
	default:
	    break;
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    const SbColor &c = editMaterial->diffuseColor[0];
    printf("Diffuse color of the selection (currently %g %g %g).\n"
	   "Type three 0..1 values; empty line or Esc cancels.\n",
	   c[0], c[1], c[2]);

    TextPrompt::activate("diffuse r g b>",
			 &SoSceneViewer::diffuseColorLineCB,
			 &SoSceneViewer::promptCancelCB, editMaterial);
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//	TextPrompt callback: applies a typed diffuse color.
//
// Use: static private
void
SoSceneViewer::diffuseColorLineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    SoMaterial	*mtl = (SoMaterial *) userData;
    float	r, g, b;
	...
	mtl->diffuseColor.setValue(r, g, b);
	mtl->diffuseColor.setIgnored(FALSE);
	TextPrompt::deactivate();
    }
    else
	fprintf(stderr, "Type three 0..1 values, e.g. '1 0.5 0'\n");
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//	TextPrompt callback: Escape cancels a color prompt.
//
// Use: static private
void
SoSceneViewer::promptCancelCB(void *)
//
////////////////////////////////////////////////////////////////////////
{
    TextPrompt::deactivate();
...
////////////////////////////////////////////////////////////////////////
{
    SoMaterial *mtl = findMaterialForAttach( NULL );
    if (mtl == NULL)
	return;

    // The typed field editor (shared with the gview port) stands in
    // for the Motif material editor.
    FieldEditor *editor = new FieldEditor(mtl);
    editor->show();
}
#else /* SDL_PORT */
void
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // The typed field editor stands in for the Motif slider set.
    FieldEditor *editor = new FieldEditor(editTransform);
    editor->show();
}
#else /* SDL_PORT */
void
...
#ifdef SDL_PORT
    // Accumulation-buffer antialiasing is not available here
    printf("Antialiasing is not available in the SDL port.\n");
#else
    if (antialiasingFlag)
...
////////////////////////////////////////////////////////////////////////
{
    ...
    printf("Ambient lighting color (currently %g %g %g).\n"
	   "Type three 0..1 values; empty line or Esc cancels.\n",
	   ambCol[0], ambCol[1], ambCol[2]);

    TextPrompt::activate("ambient r g b>",
			 &SoSceneViewer::ambientColorLineCB,
			 &SoSceneViewer::promptCancelCB, this);
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//  TextPrompt callback: applies a typed ambient lighting color.
//
//  Use: static, private
//
void
SoSceneViewer::ambientColorLineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    ...
    if (sscanf(line, "%f %f %f", &r, &g, &b) == 3) {
	sv->environment->ambientColor.setValue(r, g, b);
	TextPrompt::deactivate();
    }
    else
	fprintf(stderr, "Type three 0..1 values, e.g. '0.2 0.2 0.2'\n");
}
#else /* SDL_PORT */
void
SoSceneViewer::editAmbientColor()
...
////////////////////////////////////////////////////////////////////////
{
    const SbColor &c = currentViewer->getBackgroundColor();
    printf("Background color (currently %g %g %g).\n"
	   "Type three 0..1 values; empty line or Esc cancels.\n",
	   c[0], c[1], c[2]);

    TextPrompt::activate("background r g b>",
			 &SoSceneViewer::backgroundColorLineCB,
			 &SoSceneViewer::promptCancelCB, this);
}
...
//
// Description:
//  TextPrompt callback: applies a typed background color.
//
//  Use: static, private
//
void
SoSceneViewer::backgroundColorLineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    SoSceneViewer *sv = (SoSceneViewer *) userData;
    ...
    if (sscanf(line, "%f %f %f", &r, &g, &b) == 3) {
	sv->currentViewer->setBackgroundColor(SbColor(r, g, b));
	// keep fog color up to date with bkg color
	sv->environment->fogColor.setValue(SbColor(r, g, b));
	sv->currentViewer->scheduleRedraw();
	TextPrompt::deactivate();
    }
    else
	fprintf(stderr, "Type three 0..1 values, e.g. '0 0 0'\n");
}
#else /* SDL_PORT */
void
SoSceneViewer::editBackgroundColor()
...
#ifdef SDL_PORT
		// The SDL viewer keeps its own camera: copy the fields
		setCameraFields((SoCamera *) n);
#else
...
////////////////////////////////////////////////////////////////////////
{
    // The Motif file selection dialog becomes a typed file name
    const char *label;
    switch (fileMode) {
	case SV_FILE_OPEN:	label = "open file>";		break;
	case SV_FILE_IMPORT:	label = "import file>";		break;
	case SV_FILE_SAVE_AS:	label = "save as file>";	break;
	case SV_FILE_READ_ENV:	label = "read environment>";	break;
	case SV_FILE_SAVE_ENV:	label = "save environment>";	break;
	default:		label = "file>";		break;
    }
    printf("Type a file name; empty line or Esc cancels.\n");
    TextPrompt::activate(label, &SoSceneViewer::fileLineCB,
			 &SoSceneViewer::fileCancelCB, this);
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//	TextPrompt callback: performs the pending file operation.
//
void
SoSceneViewer::fileLineCB(void *userData, const char *line)
//
////////////////////////////////////////////////////////////////////////
{
    SoSceneViewer *sv = (SoSceneViewer *) userData;
...
void
SoSceneViewer::fileCancelCB(void *)
//
////////////////////////////////////////////////////////////////////////
{
    TextPrompt::deactivate();
...
////////////////////////////////////////////////////////////////////////
{
    printf("Printing is not available in the SDL port"
	   " (use 'W' to save the scene as an .iv file).\n");
}
#else /* SDL_PORT */
void
SoSceneViewer::print()
...
//
// Description:
//	Performs the menu action with the given SV_* id. This is the
//  Motif processTopbarEvent switch, driven by keyboard shortcuts.
//
// Use: public
//
void
SoSceneViewer::processMenuAction(int id)
//
////////////////////////////////////////////////////////////////////////
{
    switch (id) {
	...
	detachManipFromAll();
	clipboard->copy((SoPathList *)selection->getList());
	...
	break;
    case SV_EDIT_PASTE:
	clipboard->paste(pasteDoneCB, this);
	break;
    case SV_EDIT_DELETE:
	destroySelectedObjects();
	break;
	...
	switchToViewer(SV_VWR_EXAMINER);
	break;
    case SV_VIEW_WALK:
	switchToViewer(SV_VWR_WALK);
	break;
    case SV_VIEW_PLANE:
	switchToViewer(SV_VWR_PLANE);
	break;
    case SV_VIEW_FLY:
	switchToViewer(SV_VWR_FLY);
	break;
	...
	setTransparencyType(SoGLRenderAction::SCREEN_DOOR);
	break;
    ...
    case SV_VIEW_ANTIALIASING:
        setAntialiasing(! antialiasingFlag);
        break;
	...
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_TRANSFORMER) ? SV_NONE : SV_TRANSFORMER;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;

    case SV_MANIP_TRACKBALL:
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_TRACKBALL) ? SV_NONE : SV_TRACKBALL;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
	...
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_HANDLEBOX) ? SV_NONE : SV_HANDLEBOX;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
    ...
    case SV_MANIP_JACK:
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_JACK) ? SV_NONE : SV_JACK;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
	...
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip =(curManip == SV_CENTERBALL) ? SV_NONE : SV_CENTERBALL;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
    ...
    case SV_MANIP_XFBOX:
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_XFBOX) ? SV_NONE : SV_XFBOX;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
	...
	highlightRA->setVisible(FALSE); // highlight visible when no manip
	curManip = (curManip == SV_TABBOX) ? SV_NONE : SV_TABBOX;
	if ( curManipReplaces )
	    replaceAllManips( curManip );
        break;
	...
	curManipReplaces = ( curManipReplaces == TRUE) ? FALSE : TRUE;
	printf("Manips now %s existing manips on selection\n",
	       curManipReplaces ? "replace" : "do not replace");
    ...
    // Lights
    //
    case SV_LIGHT_AMBIENT_EDIT:	editAmbientColor(); break;
    case SV_LIGHT_ADD_DIRECT:	addLight(new SoDirectionalLightManip); break;
    case SV_LIGHT_ADD_POINT:	addLight(new SoPointLightManip); break;
	...
	{
	SbBool onFlag = (id == SV_LIGHT_TURN_ON);
	for (int i=0; i < lightDataList.getLength(); i++)
	    turnLightOnOff((SvLightData *) lightDataList[i], onFlag);
	turnLightOnOff( headlightData, onFlag);
	}
	break;
    case SV_LIGHT_SHOW_ALL:
    case SV_LIGHT_HIDE_ALL:
	{
	SbBool onFlag = (id == SV_LIGHT_SHOW_ALL);
	for (int i=0; i < lightDataList.getLength(); i++)
	    editLight((SvLightData *) lightDataList[i], onFlag);
	}
	break;

    } // endswitch( menu action )
}
#else /* SDL_PORT */
void
...
////////////////////////////////////////////////////////////////////////
{
	...
	if (forHeadlight) {
#ifdef SDL_PORT
	    // The typed field editor stands in for the Motif
	    // directional-light editor
	    SoLight *l = data->classPt->getHeadlight();
	    if (l != NULL) {
		FieldEditor *editor = new FieldEditor(l);
		editor->show();
	    }
#else
...
#ifdef SDL_PORT
void
SoSceneViewer::buildWidget(Widget)
//
////////////////////////////////////////////////////////////////////////
{
    // build the viewer window (the Motif version was 520x510)
    whichViewer = SV_VWR_EXAMINER;
    currentViewer = new SvViewer("SceneViewer (Examiner)", 520, 510, this);
...
{
#ifdef SDL_PORT
    printf("SceneViewer - the Open Inventor 3d scene editor (SGI, 1991-94).\n"
	   "Reads Inventor files, arranges them into a scene, edits\n"
	   "materials/transforms/lights, and saves the result. See\n"
	   "SceneViewer.about.pdf in the source distribution for the\n"
	   "original Showcase document.\n");
    return;
#else
...
////////////////////////////////////////////////////////////////////////
//
// SDL implementations of the small viewer-forwarding methods that were
// inline in the Motif header.
//

void
SoSceneViewer::viewAll()
{
    if (currentViewer != NULL)
...
void
SoSceneViewer::saveHomePosition()
{
    if (currentViewer != NULL)
...
SoCamera *
SoSceneViewer::getCamera()
{
    return (currentViewer != NULL) ? currentViewer->getCamera() : NULL;
}

void
SoSceneViewer::setHeadlight(SbBool onOrOff)
{
    if (currentViewer != NULL)
...
SbBool
SoSceneViewer::isHeadlight()
{
    if (currentViewer == NULL || currentViewer->getHeadlight() == NULL)
	return FALSE;
    return currentViewer->getHeadlight()->on.getValue();
}

SoDirectionalLight *
SoSceneViewer::getHeadlight()
{
    return (currentViewer != NULL) ? currentViewer->getHeadlight() : NULL;
}

void
SoSceneViewer::setViewing(SbBool onOrOff)
{
    if (currentViewer != NULL)
...
SbBool
SoSceneViewer::isViewing()
{
    return (currentViewer != NULL) ? (SbBool) currentViewer->isViewing() : FALSE;
}

void
SoSceneViewer::setTransparencyType(SoGLRenderAction::TransparencyType type)
{
    transpType = type;
    if (currentViewer != NULL)
    ...
    const char *name = "?";
    switch (type) {
	case SoGLRenderAction::SCREEN_DOOR:	name = "Screen Door"; break;
	case SoGLRenderAction::BLEND:		name = "Blended"; break;
	case SoGLRenderAction::DELAYED_BLEND:	name = "Delayed Blended"; break;
	case SoGLRenderAction::SORTED_OBJECT_BLEND:
						name = "Sorted Blended"; break;
	default: break;
    }
...
//
// Description:
//	Maps a key press to a menu action (the Motif menus). Returns
//  TRUE if the key was handled. Lowercase s/h/Esc stay viewer keys
//  (seek, home, viewing toggle).
//
// Use: public
//
SbBool
SoSceneViewer::handleKey(const SDL_Event *event)
//
////////////////////////////////////////////////////////////////////////
{
    SDL_Keycode	sym   = event->key.keysym.sym;
    SbBool	shift = (event->key.keysym.mod & KMOD_SHIFT) != 0;
    int		id    = -1;
	...
	return TRUE;
      ...
      // File
      case SDLK_n:	id = SV_FILE_NEW;				break;
      case SDLK_o:	id = SV_FILE_OPEN;				break;
      case SDLK_i:	id = SV_FILE_IMPORT;				break;
      case SDLK_w:	id = shift ? SV_FILE_SAVE_AS : SV_FILE_SAVE;	break;
      case SDLK_e:	id = shift ? SV_FILE_SAVE_ENV : SV_FILE_READ_ENV; break;
      case SDLK_q:	id = SV_FILE_QUIT;				break;

      // Edit (P adds a point light)
      case SDLK_p:	id = shift ? SV_LIGHT_ADD_POINT
				   : SV_EDIT_PICK_PARENT;		break;
      case SDLK_a:	id = shift ? SV_LIGHT_AMBIENT_EDIT
				   : SV_EDIT_PICK_ALL;			break;
      case SDLK_x:	id = SV_EDIT_CUT;				break;
      case SDLK_c:	id = SV_EDIT_COPY;				break;
      case SDLK_v:	id = shift ? SV_VIEW_SELECTION : SV_EDIT_PASTE;	break;
      case SDLK_BACKSPACE:
      case SDLK_DELETE:	id = SV_EDIT_DELETE;				break;

      // Viewing
      case SDLK_f:	id = SV_VIEW_FOG;				break;
      case SDLK_b:	id = SV_VIEW_BKG_COLOR;				break;
      case SDLK_t:
	if (shift) {
	    id = SV_EDITOR_TRANSFORM;
	    break;
	}
	// cycle the transparency type
	switch (transpType) {
	  case SoGLRenderAction::SCREEN_DOOR:
	    id = SV_VIEW_BLEND_TRANSPARENCY;		break;
	  case SoGLRenderAction::BLEND:
	    id = SV_VIEW_DELAY_BLEND_TRANSPARENCY;	break;
	  case SoGLRenderAction::DELAYED_BLEND:
	    id = SV_VIEW_SORT_BLEND_TRANSPARENCY;	break;
	  default:
	    id = SV_VIEW_SCREEN_TRANSPARENCY;		break;
	}
	break;

      // Editors
      case SDLK_m:	id = shift ? SV_EDITOR_COLOR : SV_EDITOR_MATERIAL; break;

      // Selection policy
      case SDLK_F5:	id = SV_SEL_SINGLE_SELECT;			break;
      case SDLK_F6:	id = SV_SEL_TOGGLE_SELECT;			break;
      case SDLK_F7:	id = SV_SEL_SHIFT_SELECT;			break;

      // Manips
      case SDLK_1:	id = SV_MANIP_TRANSFORMER;			break;
      case SDLK_2:	id = SV_MANIP_TRACKBALL;			break;
      case SDLK_3:	id = SV_MANIP_HANDLEBOX;			break;
      case SDLK_4:	id = SV_MANIP_JACK;				break;
      case SDLK_5:	id = SV_MANIP_CENTERBALL;			break;
      case SDLK_6:	id = SV_MANIP_XFBOX;				break;
      case SDLK_7:	id = SV_MANIP_TABBOX;				break;
      case SDLK_0:	id = SV_MANIP_NONE;				break;
      case SDLK_9:	id = SV_MANIP_REPLACE_ALL;			break;

      // Lights (capitals; lowercase s/h stay viewer seek/home)
      case SDLK_l:
	if (! shift)
	    return FALSE;
	id = SV_LIGHT_ADD_DIRECT;
	break;
      case SDLK_s:
	if (! shift)
	    return FALSE;
	id = SV_LIGHT_ADD_SPOT;
	break;
      case SDLK_h:
	if (! shift)
	    return FALSE;
	turnLightOnOff(headlightData, ! isHeadlight());
	printf("Headlight is now %s\n", isHeadlight() ? "on" : "off");
	return TRUE;
      case SDLK_EQUALS:
      case SDLK_PLUS:
      case SDLK_KP_PLUS:	id = SV_LIGHT_TURN_ON;			break;
      case SDLK_MINUS:
      case SDLK_KP_MINUS:	id = SV_LIGHT_TURN_OFF;			break;
      case SDLK_LEFTBRACKET:	id = SV_LIGHT_SHOW_ALL;			break;
      case SDLK_RIGHTBRACKET:	id = SV_LIGHT_HIDE_ALL;			break;
...
//
// Description:
//	Prints the keyboard shortcuts that replace the Motif menus.
//
// Use: static public
//
void
SoSceneViewer::printHelp()
//
////////////////////////////////////////////////////////////////////////
{
    printf("\n"
	"SceneViewer keys (the Motif menus):\n"
	"File:      n new scene   o open...   i import...   w save\n"
	"           W save as...  e read environment...  E save environment...\n"
	"           q quit        (printing is not available)\n"
	"Edit:      p pick parent   a pick all   x cut   c copy   v paste\n"
	"           Backspace/Del delete selection\n"
	"Viewing:   Esc toggles viewing/picking (pick objects in pick mode)\n"
	"           V view selection   f fog   t cycle transparency type\n"
	"           b background color...  (Fly/Walk/Plane viewers and\n"
	"           antialiasing are not available in the SDL port)\n"
	"Selection: F5 single   F6 toggle   F7 shift\n"
	"Editors:   m material editor   M color editor   T transform sliders\n"
	"           (typed prompts; type '<field> <value>' lines)\n"
	"Manips:    1 transformer  2 trackball  3 handlebox  4 jack\n"
	"           5 centerball  6 transformbox  7 tabbox  0 none\n"
	"           9 toggle replace-existing-manips\n"
	"Lights:    L add directional   P add point   S add spot\n"
	"           A ambient color...   H headlight on/off\n"
	"           + all on   - all off   [ show all icons   ] hide all icons\n"
	"Viewer:    (viewing mode) left-drag spin, middle-drag pan,\n"
	"           wheel dolly, s seek, h home,  ? this help\n");
}
