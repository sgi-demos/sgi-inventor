...
//
static int
linkatron_main(int argc, char **argv)
{
...
#ifdef SDL_PORT
    // Two SDL windows replace the Motif form: the parts strip on top
    // and the examiner viewer below it. (The Motif menu bar - file
    // open/save dialogs etc. - is not wired in this port.)
    buttonStrip = new ButtonStrip(NULL);
    viewer = new SoSDLExaminerViewer("Linkatron", 800, 620);
#else
...
// Zero-arg main wrapper (same as the other games): load a sample
// linkage so the browser shows a working mechanism immediately.
int
main()
{
    static char *argv[] = { (char *)"linkatron",
			    (char *)"samples/dizzyBell.iv", NULL };
    return linkatron_main(2, argv);
}
#else
int
main(int argc, char **argv)
{
    return linkatron_main(argc, argv);
}
#endif
