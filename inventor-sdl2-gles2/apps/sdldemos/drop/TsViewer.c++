...
void
TsViewer::processEvent(const SDL_Event *e)
{
    SbVec2s pos;
    switch (e->type) {
	case SDL_KEYDOWN:
	    ...
	    break;	// game keys go to the scene below

	case SDL_MOUSEBUTTONDOWN:
	    pos.setValue(e->button.x, getSize()[1] - e->button.y);
	...
	case SDL_MOUSEMOTION:
	    pos.setValue(e->motion.x, getSize()[1] - e->motion.y);
	    drag(pos);
	    break;
    }

    // Pass everything else to the render area so it reaches the scene
    // graph (the game handles its keys via an SoEventCallback).
