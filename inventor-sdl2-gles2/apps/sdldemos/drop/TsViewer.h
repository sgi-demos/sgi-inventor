...
#ifdef SDL_PORT
//
// The SDL flavor keeps the original bindings on top of the shared
// examiner-viewer machinery: left drag rotates, middle drag translates,
// Home resets the camera. Everything else reaches the scene graph
// (the game's keyboard SoEventCallback).
//
class TsViewer : public SoSDLExaminerViewer {
 public:
    TsViewer(const char *title, int width, int height)
	: SoSDLExaminerViewer(title, width, height) {}
