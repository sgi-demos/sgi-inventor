...
puck_main(int argc, char *argv[])
{
...
#ifdef SDL_PORT
    // Single process: the server runs in-process, its sensors sharing
    // the same event loop; the in-memory SoSocket layer carries the
    // datagrams (no fork under SDL/Emscripten).
    SoSDL::init();
    if (serverLaunchNeeded)
	new Server();

    // Start the player
    Player *player = new Player(serverName, NULL, useSound);
	...
	if (pid == 0) {
	    ...
	    SoXt::mainLoop();
	    return 0; // all done when the server exits
	}
	else if (pid < 0) {
	    // Error.
...
// Zero-arg main (same workaround as maze/slotcar); the browser supplies
// no arguments - solo game against the robot gun, sound on.
int
main()
{
    static char *argv[] = { (char *)"puck", NULL };
    return puck_main(1, argv);
}
#else
int
main(int argc, char *argv[])
{
    return puck_main(argc, argv);
}
#endif
