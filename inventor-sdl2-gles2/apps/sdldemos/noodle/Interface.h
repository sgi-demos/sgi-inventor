...
struct XmFileSelectionBoxCallbackStruct;
struct XmAnyCallbackStruct;
class SoSDLRenderArea;
#define SoXtViewer SoSDLRenderArea
#else
#include <Xm/Xm.h>
#endif /* SDL_PORT */
...
#ifdef SDL_PORT
    // Keyboard-driven replacement for processTopbarEvent: performs the
    // action for a PullDowns.h menu id directly (no widget state).
