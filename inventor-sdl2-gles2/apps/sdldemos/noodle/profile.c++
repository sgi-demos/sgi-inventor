...
//
#ifdef SDL_PORT
// The SDL windows report mouse-leave through their render areas
// (see noodle.c++), which call these.
void profileEditorLeft()      { if (profileLineManip) profileLineManip->removeHilights(); }
void crossSectionEditorLeft() { if (crossSectionLineManip) crossSectionLineManip->removeHilights(); }
void spineEditorLeft()        { if (spineLineManip) spineLineManip->removeHilights(); }
void twistEditorLeft()        { if (twistLineManip) twistLineManip->removeHilights(); }
#else
static void
...
#ifdef SDL_PORT
    (void)theWidget;	// leave events arrive via the render areas
