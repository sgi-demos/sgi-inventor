...
// Source-compat alias: the body of this file keeps SGI's names.
...
// no arguments - use the default card file.
int
main() {
    static char *argv[] = { (char *)"hohoho", NULL };
    int argc = 1;
#else
int
...
InitRenderArea() {
#ifdef SDL_PORT
    // Initialize Inventor and SDL; create the render area
    SoSDL::init();
    renderArea = new SoSDLRenderArea("Season's Greetings", 800, 600);
...
ParseInfo(SoPath *p) {
	...
	cb->setCallback(Wmzmzfzc, (void *)(long)(wm<<3|zm<<2|zf<<1|zc));
...
Wmzmzfzc(void *d, SoAction *) {
    int i = (int) (long) d;   // SDL port: LP64-safe
