...
#ifdef SDL_PORT
    // No files given: default to the CyberHeads that ship alongside
    // the binary (preloaded at /heads in the browser build).
    if (objectList.getLength() < 2)
    {
	static const char *defaults[] =
	    { "heads/josie.iv", "heads/paul.iv", "heads/rikk.iv" };
	for (size_t i = 0; i < sizeof(defaults)/sizeof(defaults[0]); i++)
	{
	    QuadThing *q = new QuadThing(defaults[i]);
	    if (q->getSceneGraph() != NULL)
		objectList.append(q);
	    else delete q;
	}
...
{
#ifdef SDL_PORT
    printf("Qmorf - Open Inventor 3D morphing demo (Gavin Bell, SGI).\n"
	   "Morphs geometry, materials and textures between quad-mesh\n"
	   "models (the CyberHeads). See qmorf.about.pdf in the source\n"
...
#ifdef SDL_PORT
//
// Keyboard replaces the Motif strip: two Animate toggles, two
// scrub sliders, About and Quit.
//
static void
printHelp()
{
    printf("Qmorf keys:\n"
	   "  c          toggle color morphing\n"
	   "  s          toggle shape morphing\n"
	   "  left/right scrub color morph (stops its animation)\n"
	   "  up/down    scrub shape morph (stops its animation)\n"
	   "  a          about\n"
	   "  h          this help\n"
	   "  q          quit\n");
}
...
static bool
handleQmorfKey(SDL_Keycode key)
{
    const double step = 0.05;
    switch (key) {
      case SDLK_c:
	if (whatIsAnimating[COLOR]) stopAnimating(COLOR);
	else startAnimating(COLOR);
	return true;
      case SDLK_s:
	if (whatIsAnimating[SHAPE]) stopAnimating(SHAPE);
	else startAnimating(SHAPE);
	return true;
      case SDLK_LEFT:
	stopAnimating(COLOR);
	setValue(COLOR, animationTime[COLOR] - step + objectList.getLength());
	return true;
      case SDLK_RIGHT:
	stopAnimating(COLOR);
	setValue(COLOR, animationTime[COLOR] + step);
	return true;
      case SDLK_DOWN:
	stopAnimating(SHAPE);
	setValue(SHAPE, animationTime[SHAPE] - step + objectList.getLength());
	return true;
      case SDLK_UP:
	stopAnimating(SHAPE);
	setValue(SHAPE, animationTime[SHAPE] + step);
	return true;
      case SDLK_a:
...
class QmorfViewer : public SoSDLExaminerViewer {
  public:
    QmorfViewer(const char *t, int w, int h) : SoSDLExaminerViewer(t, w, h) {}
    virtual void processEvent(const SDL_Event *e) {
	if (e->type == SDL_KEYDOWN && handleQmorfKey(e->key.keysym.sym))
	    return;
...
buildUI(Widget, SoNode *root, SoPerspectiveCamera *)
{
    QmorfViewer *viewer = new QmorfViewer("Qmorf", 600, 600);
    // setSceneGraph adopts the camera main() already put in root.
