...
SoGLLazyElement::setColorMaterialElt(SbBool value)
{
    ...
    // Emscripten's legacy GL emulation has no GL_COLOR_MATERIAL: its
    // lighting shader reads diffuse only from glMaterialfv. Force the
    // glMaterial path so diffuse colors reach the shader. (Same patch
    // as SoLazyElement::setColorMaterialElt; this override is the one
    // actually used during GL rendering.)
    value = FALSE;
#endif
