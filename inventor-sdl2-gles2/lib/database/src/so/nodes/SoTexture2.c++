...
#else
// SDL/Emscripten port: libjpeg unavailable in this build; JPEG textures
// fail to load with a debug error (GIF and SGI .rgb still work).
SbBool ReadJPEGImage(const SoInput&, int &, int &, int &, unsigned char *&)
{
    return FALSE;
}
#endif /* IV_NO_JPEG */
