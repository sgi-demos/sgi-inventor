...
/* SDL port: window/context handles are SDL types. Forward-declared so
 * this header does not force SDL.h on all of Inventor. */
struct SDL_Window;
...
SoEXTENDER class SoWindowElement : public SoElement {
  ...
  public:
    ...
    static void		set(SoState *state, SDL_Window *window,
			    SDL_GLContext context, SoGLRenderAction *glAction);

    // Returns the current window, context and glRenderAction
    static void	get(SoState *state, SDL_Window *&window,
			    SDL_GLContext &context, SoGLRenderAction *&glAction);
