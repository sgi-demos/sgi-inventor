...
| M11c | **SceneViewer** (Beier/Dumesny/Mott/Isaacs's 3d scene editor, the heaviest Motif surface, 6,400 lines): all seven pulldown menus became single-key shortcuts dispatched through the same SV_* action ids; the four Xt editor components (material editor, color editor, transform slider set, directional-light editor) all became the gview port's typed field editor; the file selection dialog a typed file-name prompt; SoXtClipboard an in-process SoByteStream buffer feeding the original pasteDone(); ambient/background color editors one-line "r g b" prompts. The manipulator machinery - transformer/trackball/handlebox/jack/centerball/transformbox/tabbox on selections, light manips, remove-manips-before-write - is pure Inventor and runs unchanged (libSoSDL gained viewer finish callbacks for the TabBox scale tabs, plus a getBackgroundColor). Only the Examiner viewer exists (Fly/Walk/Plane print a note). Verified: gl4es vs desktop 0.0000%; scripted native run picks the jack-in-the-box, attaches a trackball, edits the material to green through the typed editor, and saves - the file has the new material and no manip nodes; browser boot + pick + trackball attach/detach |
...
- All LGPL demos from apps/demos are now ported (SceneViewer was the
  last, M11c).
