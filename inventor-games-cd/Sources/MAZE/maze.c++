...
#define GRAVITATIONAL_CONSTANT 45.0
...
#define ROTATION_LIMIT         0.16
//#define MAX_ELAPSED_TIME       0.06
#define MAX_ELAPSED_TIME       0.12
...
////////////////////////////////////////////////////////////////////////
{
	...
	if (!isMouseDown) {
	    timer->unschedule();
            alarmTimer->setTimeFromNow(SbTime(2.0));
            alarmTimer->schedule();
        }
        return;
    }
...
//
// Description:
//    Callback routine for alarm sensor to start another game.
//
// Use: private

static void
resetBall( void *, SoSensor * )
...
main(int argc, char *argv[])
{
    ...
    victory->setFilename("./tag3.aiff");
    ballSound = new PlayClass("BallSound");
    ballSound->setFilename("./glass_break.aiff");
#endif
    ...
    // Setup a timer sensor to animate the ball and reset a new game
    timer = new SoTimerSensor(animateBall, NULL);
    timer->setInterval(SbTime(0.03));
    alarmTimer = new SoAlarmSensor(resetBall, NULL);
