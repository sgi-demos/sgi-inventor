...
Sound::playOnce(const char *soundName, float waitTime)
{
    ...
    data->semaphore = usnewsema(arena, 1);
...
// = as is).  The sound begins turned off.
//
////////////////////////////////////////////////////////////////////////////
...
Sound::initContinuous(const char *soundName, float pitch)
{
    ...
    data->semaphore = usnewsema(arena, 0);
    ...
    if (! initiate(data)) {
	delete data;
        fprintf(stderr, "Could not initialize sound\n");
	return -1;
    }
...
////////////////////////////////////////////////////////////////////////////
//
// Pause or resume a continuous sound.
//
////////////////////////////////////////////////////////////////////////////
...
{
    SoundData *data = continuousSounds[soundId];

    // Make sure id is valid
    if (soundId < 0 || data == NULL)
	return;
    if (data->isPlaying == playFlag)
        return;

    data->isPlaying = playFlag;

    // Turn the sound back on
    if (playFlag) {
        data->pitch = newPitch;
        usvsema(data->semaphore);
    }
    else {
        uspsema(data->semaphore);
    }
}
...
Sound::initiate(SoundData *data)
{
    ...
    if (port == NULL) {
	ALfreeconfig(config);
        fprintf(stderr, "No audio ports.  Can't play sound.\n");
	return FALSE;
    }
...
{
    prctl(PR_TERMCHILD);
    sigset(SIGHUP, (void (*)(int ...)) &killHandler);
    ...
    // 1/20th of a second at a time.
    long framesPerBuf = data->sampleRate / 20;
	...
	else if (uscpsema(data->semaphore)) {
	    data->prevPitch = pitch;
	    pitch = data->pitch;
	    usvsema(data->semaphore);
